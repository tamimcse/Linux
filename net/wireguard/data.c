...
{
	/* TODO: do we need to extract endpoint BEFORE this, since the cow functions? */
...
static void finish_decrypt_packet(struct decryption_ctx *ctx)
{
	...
	ret = socket_endpoint_from_skb(&endpoint, ctx->skb);
	if (unlikely(ret < 0))
		goto err;
	...
	skb_reset(ctx->skb);
	ctx->callback(ctx->skb, ctx->keypair->entry.peer, &endpoint, used_new_key, 0);
	return;
